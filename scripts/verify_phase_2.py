...
(English, Hindi, Marathi, Mathematics, EVS/GK), absence of Phase 3+ forbidden features,
and runs pytest unit tests.
"""
import subprocess
import sys
...
def verify_no_phase_3_features(root: Path) -> bool:
    print("[3/5] Checking for forbidden Phase 3+ implementations...")
    forbidden_keywords = [
        "suman_ai",
        "voice_engine",
        "speech_to_text",
            ...
            if file_path.is_file():
                ...
                for kw in forbidden_keywords:
                    if kw in content.lower():
                        print(f"FAIL: Forbidden Phase 3+ keyword '{kw}' found in {file_path}")
                        return False

    print("No Phase 3+ features check: PASS")
    return True
